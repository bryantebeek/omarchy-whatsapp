...
MUTATIONS = [
    ...
    mutation("model-message-date-key", M, TM, 'date.getMonth() + "-" + date.getDate()', 'date.getMonth() + "-" + 1'),
    mutation("model-message-date-label", M, TM, '"dddd, d MMMM yyyy"', '"d MMM"'),
    ...
    mutation("model-link-www-detection", M, TM, "var pattern = /(?:https?:\\/\\/|www\\.)[^\\s<>\"']+|@[0-9]+/gi", "var pattern = /(?:https?:\\/\\/)[^\\s<>\"']+|@[0-9]+/gi"),
    mutation("model-link-www-scheme", M, TM, 'parts.url.indexOf("www.") === 0', 'parts.url.indexOf("www.") !== 0'),
    mutation("model-mention-contact-match", M, TM, "if (!matched) continue", "if (matched) continue"),
    mutation("model-mention-alias-match", M, TM, 'matched = address.split("@")[0].replace(/[^0-9]/g, "") === digits', 'matched = address.split("@")[0].replace(/[^0-9]/g, "") !== digits'),
    mutation("model-mention-known-only", M, TM, "if (!mention) continue", "if (mention) continue"),
    mutation("model-mention-email-boundary", M, TM, "if (/[A-Za-z0-9_]/.test(before) || /[A-Za-z0-9_]/.test(after)) continue", "if (/[A-Za-z0-9_]/.test(before) && /[A-Za-z0-9_]/.test(after)) continue"),
    ...
    mutation("service-direct-chat-identity", S, TS,
             'jid: value,\n      name: String(name || "")',
             'jid: "mutated@s.whatsapp.net",\n      name: String(name || "")'),
    mutation("service-selected-chat-refresh-preservation", S, TS,
             'if (!selected || !selected.jid) return incoming',
             'if (selected && selected.jid) return incoming'),
    mutation("service-daemon-chat-replaces-synthetic", S, TS,
             'String((incoming[i] || {}).jid || "") === selectedJid',
             'String((incoming[i] || {}).jid || "") !== selectedJid'),
    mutation("service-daemon-setup-busy-guard", S, TS,
             "if (daemonSetupBusy) return false",
             "if (!daemonSetupBusy) return false"),
    mutation("service-daemon-detail-prefix", S, TS,
             'if (detail.indexOf("setup: ") === 0) detail = detail.substring(7)',
             'if (detail.indexOf("setup: ") !== 0) detail = detail.substring(7)'),
    mutation("service-daemon-setup-success", S, TS,
             "daemonSetupBusy = false\n    daemonRuntimeChecked = true\n    daemonRuntimeReady = exitCode === 0",
             "daemonSetupBusy = false\n    daemonRuntimeChecked = true\n    daemonRuntimeReady = exitCode !== 0"),
    ...
    mutation("service-avatar-self", S, TS,
    ...
    mutation("service-send-disconnected", S, TS, "if (!socket || !socket.connected || !protocolCompatible) return 0", "if (socket || !socket.connected || !protocolCompatible) return 0"),
    ...
    mutation("service-text-acceptance-identity", S, TS,
             'String(frame.delivery_id || "") === String(pending.delivery_id || "")',
             'String(frame.delivery_id || "") !== String(pending.delivery_id || "")'),
    mutation("service-old-generation-rejected", S, TS,
             "generation < daemonGeneration", "generation > daemonGeneration"),
    mutation("service-old-sequence-rejected", S, TS,
             "sequence <= Number(resourceSequences[key] || 0)",
             "sequence > Number(resourceSequences[key] || 0)"),
    mutation("service-focused-read", S, TS,
             "if (panelVisible && panelFocused)\n      send(\"mark_read\"",
             "if (panelVisible || panelFocused)\n      send(\"mark_read\""),
    mutation("service-message-invalidation-key", S, TS,
             'key === selectedChatJid && requestMessages(key, true)',
             'key !== selectedChatJid && requestMessages(key, true)'),
    mutation("service-text-outbox-failed-selection", S, TS,
             'String(textOutboxEntries[i].status || "") === "failed"',
             'String(textOutboxEntries[i].status || "") === "queued"'),
    mutation("service-text-retry-command", S, TS,
             'send("retry_text_message", { delivery_id: deliveryId })',
             'send("discard_text_message", { delivery_id: deliveryId })'),
    mutation("service-text-discard-sending-guard", S, TS,
    ...
    mutation("service-local-recording-state", S, TS, 'state: "recording"\n    })) return false', 'state: "typing"\n    })) return false'),
    mutation("service-protocol-handshake", S, TS, "received !== protocolVersion", "received === protocolVersion"),
    mutation("service-protocol-send-gate", S, TS, "!socket.connected || !protocolCompatible", "!socket.connected && !protocolCompatible"),
    mutation("service-voice-recording-id-validation", S, TS, "value.length > 0 && value.length <= 80", "value.length > 80 && value.length <= 80"),
    mutation("service-voice-outbox-status-validation", S, TS, 'status !== "sending" && status !== "failed"', 'status !== "sending" && status === "failed"'),
    mutation("service-voice-duration-minimum", S, TS, "duration < 250 || duration > 15 * 60 * 1000", "duration < 0 || duration > 15 * 60 * 1000"),
    mutation("service-voice-send-command", S, TS, 'voiceMessageRequestId = send("send_voice_message"', 'voiceMessageRequestId = send("send_message"'),
    mutation("service-voice-error-retention", S, TS, 'var recordingId = voiceMessageRequestRecordingId\n    if (frame.event === "error") {', 'var recordingId = voiceMessageRequestRecordingId\n    if (frame.event === "sent") {'),

    mutation("panel-paired-state", P, TP, 'readonly property bool paired: service && service.connectionState === "connected"', 'readonly property bool paired: service && service.connectionState !== "connected"'),
    ...
    mutation("panel-mention-link-routing", P, TP, 'if (value.indexOf(prefix) !== 0) {', 'if (value.indexOf(prefix) === 0) {'),
    mutation("panel-mention-chat-entry", P, TP,
             "service.ensureDirectChat(contact.jid, contact.name)",
             'service.ensureDirectChat("mutated@s.whatsapp.net", contact.name)'),
    mutation("panel-daemon-setup-action", P, TP,
             "root.service.setupDaemonRuntime()",
             "root.service.retryDaemon()"),
    mutation("panel-submit-composer-text", P, TP,
             "if (!service || !service.sendMessage(composer.text)) return",
             'if (!service || !service.sendMessage("")) return'),
    mutation("panel-clear-matching-accepted-draft", P, TP,
             '&& composer.text === String(text || "")) composer.text = ""',
             '&& composer.text !== String(text || "")) composer.text = ""'),
    mutation("panel-accepted-draft-chat-identity", P, TP,
             'root.service.selectedChatJid === String(chatJid || "")',
             'root.service.selectedChatJid !== String(chatJid || "")'),
    mutation("panel-text-outbox-failure-label", P, TP,
             '"Message failed  " + preview', '"Message sent  " + preview'),
    mutation("panel-input-height-buttons-square", P, TP, "width: root.snapToDevicePixel(controlHeight)\n    height: width", "width: root.snapToDevicePixel(controlHeight) + 1\n    height: width"),
    mutation("panel-square-button-icon-horizontal-center", P, TP, "horizontalAlignment: Text.AlignHCenter\n      verticalAlignment: Text.AlignVCenter", "horizontalAlignment: Text.AlignLeft\n      verticalAlignment: Text.AlignVCenter"),
    mutation("panel-square-button-icon-vertical-center", P, TP, "horizontalAlignment: Text.AlignHCenter\n      verticalAlignment: Text.AlignVCenter", "horizontalAlignment: Text.AlignHCenter\n      verticalAlignment: Text.AlignTop"),
    mutation("panel-voice-stop-action", P, TP, 'voiceRecordingStopAction = sendRecording === true ? "send" : "discard"', 'voiceRecordingStopAction = sendRecording !== true ? "send" : "discard"'),
    mutation("panel-voice-recording-control-order", P, TP, 'id: voiceRecordingControls\n                      objectName: "voiceRecordingControls"', 'id: voiceRecordingControls\n                      objectName: "voiceRecordingControls"\n                      layoutDirection: Qt.RightToLeft'),
    ...
    mutation("panel-image-bubble-style", P, TP, '&& (messageDelegate.mediaData.kind === "video"\n                            || messageDelegate.mediaData.kind === "location")', '&& (messageDelegate.mediaData.kind === "image"\n                            || messageDelegate.mediaData.kind === "video"\n                            || messageDelegate.mediaData.kind === "location")'),
    mutation("panel-image-corner-radius", P, TP, "radius: bubble.radius\n                              visible: false", "radius: 0\n                              visible: false"),
    mutation("panel-image-mask-enabled", P, TP, "layer.enabled: mediaPreviewCard.isImage", "layer.enabled: false"),
    mutation("panel-media-top-margin", P, TP, "readonly property real topMargin: Style.space(8)", "readonly property real topMargin: 0"),
    ...
    mutation("panel-poll-option-action", P, TP, "onClicked: messageDelegate.togglePollOption(\n                                      pollOption.index)", "onClicked: {}"),
    mutation("panel-poll-progress-height", P, TP,
             "height: Style.space(9)\n                                    radius: height / 2",
             "height: Style.space(2)\n                                    radius: height / 2"),
    mutation("panel-poll-option-radius", P, TP,
             "radius: bubble.radius\n                                    foreground: root.foreground",
             "radius: Style.cornerRadius\n                                    foreground: root.foreground"),
    mutation("panel-poll-subtle-border", P, TP,
             "root.foreground.g, root.foreground.b, 0.10)",
             "root.foreground.g, root.foreground.b, 0.60)"),
    mutation("panel-poll-progress-inset", P, TP,
             "anchors.bottom: parent.bottom\n                                    height: Style.space(9)",
             "anchors.bottom: parent.bottom\n                                    anchors.leftMargin: Style.space(10)\n                                    anchors.rightMargin: Style.space(10)\n                                    anchors.bottomMargin: Style.space(7)\n                                    height: Style.space(9)"),
    mutation("panel-poll-voter-list", P, TP,
             "model: pollOption.voterJids\n\n                                        delegate: CrispBorderSurface",
             "model: []\n\n                                        delegate: CrispBorderSurface"),
    mutation("panel-poll-avatar-overlap", P, TP,
             "readonly property int avatarStride: Style.space(14)",
             "readonly property int avatarStride: Style.space(24)"),
    ...
    mutation("panel-date-divider-boundary", P, TP, '&& (!previousMessage || messageDateKey\n                          !== Model.messageDateKey(previousMessage.timestamp))', '&& (!previousMessage || messageDateKey\n                          === Model.messageDateKey(previousMessage.timestamp))'),
    mutation("panel-date-divider-lines", P, TP, 'readonly property real lineWidth: Math.max(0,\n                          width - dateDividerLabelBackground.width\n                            - Style.space(48)) / 2', 'readonly property real lineWidth: 0'),
    mutation("panel-date-divider-bubble-offset", P, TP, "y: dateDivider.height", "y: 0"),
    mutation("panel-date-divider-font-size", P, TP, "font.pixelSize: root.messageMetaFontSize + 2", "font.pixelSize: root.messageMetaFontSize"),
    mutation("panel-date-divider-bottom-margin", P, TP, "readonly property real bottomSpacing: Style.space(12)", "readonly property real bottomSpacing: Style.space(0)"),
