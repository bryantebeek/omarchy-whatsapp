...
printf '%s\n' 'pub fn existing_entry_point() {}' >"$fixture_dir/crates/daemon/src/main.rs"
git -C "$fixture_dir" add crates/daemon/src/main.rs
git -C "$fixture_dir" commit --quiet --message daemon-baseline
printf '%s\n' 'pub fn changed_entry_point() {}' >>"$fixture_dir/crates/daemon/src/main.rs"
printf 'SF:%s\nDA:1,1\nDA:2,0\nend_of_record\n' \
  "$fixture_dir/crates/daemon/src/main.rs" >"$lcov_file"
if COVERAGE_REPO_DIR="$fixture_dir" \
  "$repo_dir/scripts/check-diff-coverage.sh" "$lcov_file" HEAD >/dev/null 2>&1; then
  echo "Diff coverage accepted an uncovered daemon entry-point line." >&2
  exit 1
fi
